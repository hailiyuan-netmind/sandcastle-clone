...
- Full game loop: Chinese HUD, wave scheduler with growing waves, a rising tide after every wave, plantable flag with washed/undermined detection (3x3 neighborhood water check), a game-over panel with run stats and restart, and procedurally synthesized audio (ocean noise swelling with each wave, sand rustle while sculpting) with zero sound assets
